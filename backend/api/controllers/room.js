...
export const AvailityupdateRoom= async(req,res,next) => {
    try {
        await Room.updateOne(
            {"roomnum._id":req.params.id},
            {
                $push:{
                    "roomnum.$.unavialabledate":req.body.dates
                }
            }
        )
        res.status(202).send("room number update to date");
